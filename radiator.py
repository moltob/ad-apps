...
class RadiatorApp(MyHomeAssistantApp):
    ...
    entity_time_comfort_start: appdaemon.entity.Entity
    entity_time_comfort_stop: appdaemon.entity.Entity
    entity_temperature_comfort: appdaemon.entity.Entity
    entity_temperature_eco: appdaemon.entity.Entity

    async def initialize(self):
        await super().initialize()
        await self.entity_window.listen_state(self.control_radiator)
    ...
    async def control_radiator(self, *args, **kwargs):
        ...
        if await self.entity_window.get_state() == 'on':
        ...
        comfort_start = datetime.time.fromisoformat(
            await self.entity_time_comfort_start.get_state()
        )
        comfort_stop = datetime.time.fromisoformat(await self.entity_time_comfort_stop.get_state())

        if comfort_start <= now < comfort_stop:
            await self.set_temperature(
                float(await self.entity_temperature_comfort.get_state()), 'Comfort mode'
            )
        else:
            await self.set_temperature(
                float(await self.entity_temperature_eco.get_state()), 'Eco mode'
            )

    async def set_temperature(self, target_temperature: float, reason: str):
        apparent_temperature = float(await self.entity_radiator.get_state('current_temperature'))
        actual_temperature = float(await self.entity_temperature.get_state())
        ...
        await self.entity_radiator.call_service('set_temperature', temperature=set_temperature)
